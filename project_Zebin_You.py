...
if page=='b0':
    ...
    st.markdown('<p style="font-size:28px; font-family:\'Times New Roman\', serif; color:#333333;">'
    ...
    cou=st.radio('Select a country to view CO₂ Emissions per Year',('USA', 'Germany'),index=0 if st.session_state.country_selected=='USA' else 1,key='cou')
    fig, ax=plt.subplots(figsize=(12, 6))
    for country in co2['Country'].unique():
        xf=co2.loc[co2['Country']==country]
        ax.plot(xf['Year'], xf['Value'], alpha=1, color='blue' if country==cou else 'gray', linewidth=1.2 if country==cou else 0.8, label=cou if country==cou else None)
